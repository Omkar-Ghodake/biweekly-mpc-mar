...
const ChatBox = ({
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="w-[30vw] h-[90vh] bg-gradient-to-b from-blue-900 to-blue-950 rounded-xl p-4 text-white shadow-lg flex flex-col relative"
    >
      <RxCross2
        className="absolute right-5 text-2xl cursor-pointer"
        onClick={() => setScreen("floatingIcon")}
      />
      {screen === "greeting" ? (
      ...
      <div className="absolute bottom-0 left-0 w-full px-4 pb-4 bg-transparent">
        ...
        <div className="flex items-center space-x-1">
          ...
          <button
            onClick={sendMessage}
            className="bg-white p-2 w-10 h-10 flex justify-center items-center shadow-md rounded-lg"
          >
